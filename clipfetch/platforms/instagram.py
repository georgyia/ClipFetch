...
class Instagram(Platform):
    ...
    def collect_target(self, context, target, quality, count, on_clip, already_have):
            ...
            try:
                if self.host in response.url and "application/json" in \
                    ...
                    for clip in self.find_clips(response.json(), quality):
                        urls_by_code.setdefault(clip.ident, clip.video_url)
            except Exception:
                pass
        ...
        for code in codes:
            if len(clips) >= count:
                break
            url = urls_by_code.get(code) or self._resolve_permalink(page, code, urls_by_code)
            if url:
                clip = Clip(self.key, ident=code, video_url=url)
                clips.append(clip)
                on_clip(clip)
        return clips
    ...
    def _resolve_permalink(self, page, code, urls_by_code) -> Optional[str]:
        """Open a reel permalink so its playable URL is captured, then return it."""
        page.goto(f"{_HOME}reel/{code}/", wait_until="domcontentloaded")
        page.wait_for_timeout(300)
        deadline = _PERMALINK_TIMEOUT_MS
        while code not in urls_by_code and deadline > 0:
            page.wait_for_timeout(200)
            deadline -= 200
        return urls_by_code.get(code)
